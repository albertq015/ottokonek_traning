package com.otto.mart.ui.activity.ppob.product.paketData
...
class PpobPaketDataProductInputActivity : PpobActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_ppob_paket_data_product_input)
    ...
    private fun initLabel() {
        when (mPpobProductType?.code) {
            AppKeys.PPOB_TYPE_PAKET_DATA -> {
                etCustNumber.hint = getString(R.string.ppob_hint_id_pelanggan)
            }
    ...
    private fun initBanner() {
        ...
        when (mPpobProductType?.code) {
            AppKeys.PPOB_TYPE_PAKET_DATA -> imgSrc = R.drawable.ppob_banner_paket_data
        }
    ...
    private fun validateInputForm(): Boolean {
        if (mIsValidationEnable) {
            ...
            if ((mPpobProductType.code.equals(AppKeys.PPOB_TYPE_TELKOM))
                    || (mPpobProductType.code.equals(AppKeys.PPOB_TYPE_TELKOM_INTERNET))) {
                if (!mFormValidation!!.isRequired(etCustNumber.text.toString())) {
                    ...
                    status = false
                } else if (!mFormValidation!!.isValidPhone(etCustNumber.text.toString())) {
                    tvCustNumberError.text = getString(R.string.ppob_msg_phone_not_found)
                    tvCustNumberError.visible()
                    status = false
