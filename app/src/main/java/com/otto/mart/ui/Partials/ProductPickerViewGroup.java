...
import com.otto.mart.ui.Partials.adapter.RadioishRecycleAdapter;
...
public class ProductPickerViewGroup extends LinearLayout {
    ...
    public void initLayout(@Nullable String title, List<ProductIconModel> models, int parentLayoutId, int childLayoutId, LinearLayoutManager lm, RecyclerView.ItemDecoration decorator, boolean isPaketData, boolean isAsu, boolean isTitleOnly) {
        if (!isInitialized) {
            pLayoutID = parentLayoutId;
            cLayoutID = childLayoutId;
            isInitialized = !isInitialized;
    ...
    private void initContent(List<ProductIconModel> models, LinearLayoutManager lm, RecyclerView.ItemDecoration decorator) {
        adapter = new RadioishRecycleAdapter(models, cLayoutID, 0, 0, R.color.ocean_blue, R.color.color_white, isPaketData, isAsu, isTitleOnly);
