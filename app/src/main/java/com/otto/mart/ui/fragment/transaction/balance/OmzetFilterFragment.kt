...
import kotlinx.android.synthetic.main.fragment_omzet_filter.*
import java.util.*
 ...
 */
class OmzetFilterFragment : BottomSheetDialogFragment(), DatePickerDialog.OnDateSetListener {

    var paymentTypeList = mutableListOf<OmzetFilter>()
    var transactionTypeList = mutableListOf<OmzetFilter>()

    lateinit var applyFilterOmzet: (startDate: String, endDate: String, paymentTypeList: MutableList<OmzetFilter>,
                                    transactionTypeList: MutableList<OmzetFilter>) -> Unit
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        val view = inflater.inflate(R.layout.fragment_omzet_filter, container, false)
    ...
    private fun initView() {
        ...
        btnSubmit.setOnClickListener {
            if (::applyFilterOmzet.isInitialized) {
                applyFilterOmzet(tvStartDate.text.toString(), tvEndDate.text.toString(), paymentTypeList, transactionTypeList)
                dismiss()
            }
        }
    }
    ...
    private fun displayPaymentType() {
        if (paymentTypeList.size == 0) {
            val all = OmzetFilter("All", "",true)
            paymentTypeList.add(all)

            val qris = OmzetFilter("QR", "qr",false)
            paymentTypeList.add(qris)

            val withdrawal = OmzetFilter("Withdrawal", "withdrawal",false)
            paymentTypeList.add(withdrawal)
        }

        activity?.let {
            val adapter = OmzetFilterAdapter(it, paymentTypeList)
            adapter.itemSelected = ::paymentTypeSelected
            rvPaymentType.adapter = adapter
        }
    }

    fun paymentTypeSelected(omzetFilter: OmzetFilter, position: Int) {
        var currentSelectedStatus = omzetFilter.isSelected
        omzetFilter.isSelected = !currentSelectedStatus
        paymentTypeList[position] = omzetFilter
        rvPaymentType.adapter?.notifyItemChanged(position)
    }
