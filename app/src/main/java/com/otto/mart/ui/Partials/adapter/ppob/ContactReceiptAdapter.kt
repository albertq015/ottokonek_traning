...
class ContactReceiptAdapter(private val mContext: Context,
var mDataset: List<WidgetBuilderModel>
) : RecyclerView.Adapter<ContactReceiptAdapter.ViewHolder>() {
        ...
        if (orientation == HORIZONTAL) {
            if (item.key.equals("petunjuk penggunaan", true)) {
                ...
                var labelNoResi = item.key.replace(".", "")
                    .replace(" ", "")
                if ((labelNoResi.equals("noresi", true)) || (labelNoResi.equals(
                        "receiptnumber",
                        true
                    ))
                ) {
