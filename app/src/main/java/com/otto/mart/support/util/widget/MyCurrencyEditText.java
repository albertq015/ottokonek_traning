package com.otto.mart.support.util.widget;
 ...
 */
public class MyCurrencyEditText extends androidx.appcompat.widget.AppCompatEditText {
    ...
    public MyCurrencyEditText(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, faranjit.currency.edittext.R.styleable.currencyEditText, 0, 0);

        try {
            if (a.getString(faranjit.currency.edittext.R.styleable.currencyEditText_groupDivider) != null) {
                this.mGroupDivider = a.getString(faranjit.currency.edittext.R.styleable.currencyEditText_groupDivider).charAt(0);
                this.groupDivider = mGroupDivider;
            }

            if (a.getString(faranjit.currency.edittext.R.styleable.currencyEditText_monetaryDivider) != null) {
                this.mMonetaryDivider = a.getString(faranjit.currency.edittext.R.styleable.currencyEditText_monetaryDivider).charAt(0);
                this.monetaryDivider = mMonetaryDivider;
            }

            if (a.getString(faranjit.currency.edittext.R.styleable.currencyEditText_locale) == null)
                this.locale = getDefaultLocale();
            else
                this.mLocale = a.getString(faranjit.currency.edittext.R.styleable.currencyEditText_locale);

            if (a.getString(faranjit.currency.edittext.R.styleable.currencyEditText_showSymbol) != null)
                this.mShowSymbol = a.getBoolean(faranjit.currency.edittext.R.styleable.currencyEditText_showSymbol, false);
    ...
    private TextWatcher onTextChangeListener = new TextWatcher() {
        ...
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            ...
            setText(text);
            if (text.length() <= 14) {
//                if (text.length() > start)setSelection(start);
//                else
                try {
    ...
    private String format(String text) throws ParseException {
        ...
        try {
            if (mShowSymbol)
                format = numberFormat.format(Double.parseDouble(text) / Math.pow(10, fractionDigit));
            else
                format = numberFormat.format(Double.parseDouble(text) / Math.pow(10, fractionDigit)).replace(currencySymbol, "");
